...
<?php
    ...
    function edit() {
        ...
        $result = $conn->query(sprintf("SELECT * FROM food_order WHERE name= '%s' AND order_id = '%s'", $_SESSION["username"], filter($_POST['id']))); //user exists
        ...
        if (!empty($errors)) {
            ...
            $conn->query(sprintf("UPDATE food_order SET food = '%s' WHERE order_id = '%s'", filter($_POST['order']), filter($_POST['id'])));
